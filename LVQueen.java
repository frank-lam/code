    ...
    {  
        ...
        {  
            ...
            {  
                ...
                System.out.println("place("+k+")="+place(k));
                if(place(k))  
                {  
                    if(rnd.nextInt(++count) == 0)  
                        {  
                            ...
                            //break;
                            //有break如果第一次找到合适的列place(k)满足，
                            //那么此时random（1）==0恒成立，遇到下面的break，就把皇后放置在这个位置。
                            //如果这种放置皇后的方案不可行，下次循环还会执行同样的，故一直循环调不出来找不到方案。
                            //即剩下的所有皇后放置不了的可能性增大。  
                            
                            //没有break，会一直试探for循环结束。
                            //x[k]会在随机的选择当前可以放置的位置中for循环最后一个满足的列。
                            //那么后面如果n-1个皇后放置不了的可能性减小。  
                        }  
                }  
            }  
            if(count > 0)   
